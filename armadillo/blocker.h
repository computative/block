...
private:
    ...
    double estimate(arma::vec& x, unsigned long long& n);

    // performs blocking transformation
    void transform(arma::vec& X, arma::vec& x, unsigned long long& n);

    // estimates gamma_h(1) for all h
    double gamma1(arma::vec& X, unsigned long long& n);

    // estimates gamma_h(0) for all h
    double var(arma::vec& X, unsigned long long& n);
};
